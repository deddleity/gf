	...
	<body>
		...
		<?php foreach ($mbpro_font_families as $name => $value) { ?>
			<?php if ($value != '') { ?>
				...
				<style type="text/css">
					.maxbutton-link .maxbutton .text.font-<?php echo $count ?> { font-family: <?php echo $value ?>; }
				</style>
				
				<div class="maxbutton-wrapper">
					<a class="maxbutton-link" href="#">
						<div class="maxbutton">
							<div class="text font-<?php echo $count ?>"><?php _e('This is', 'maxbuttons-pro') ?> <?php echo $name ?></div>
						</div>
					</a>
				</div>
				<?php $count++; ?>
